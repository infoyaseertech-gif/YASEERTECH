// ============================================
//   YASEERTECH - INVOICE (SUPABASE) v3
//   Invoices = Sales merged
// ============================================

const INV_URL  = "https://winlgtflhwiaraniwpts.supabase.co";
const INV_KEY  = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndpbmxndGZsaHdpYXJhbml3cHRzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkxODUzMjUsImV4cCI6MjA5NDc2MTMyNX0.Co0PPWjMtNeIIa8GSqkulgytzhwbm33p718HsPMECKU";
const INV_REST = INV_URL + "/rest/v1";

async function invGet(ep){const r=await fetch(INV_REST+ep,{headers:{"apikey":INV_KEY,"Authorization":"Bearer "+INV_KEY,"Content-Type":"application/json"}});const t=await r.text();return t?JSON.parse(t):[];}
async function invPost(ep,data){const r=await fetch(INV_REST+ep,{method:"POST",headers:{"apikey":INV_KEY,"Authorization":"Bearer "+INV_KEY,"Content-Type":"application/json","Prefer":"return=representation"},body:JSON.stringify(data)});const t=await r.text();return t?JSON.parse(t):[];}
async function invPatch(ep,data){const r=await fetch(INV_REST+ep,{method:"PATCH",headers:{"apikey":INV_KEY,"Authorization":"Bearer "+INV_KEY,"Content-Type":"application/json","Prefer":"return=representation"},body:JSON.stringify(data)});const t=await r.text();return t?JSON.parse(t):[];}
async function invDelete(ep){await fetch(INV_REST+ep,{method:"DELETE",headers:{"apikey":INV_KEY,"Authorization":"Bearer "+INV_KEY}});}
...
function addInvoiceItem(){
    ...
    row.innerHTML=`<div class="inv-item-fields">
...
function removeItem(id){const row=document.getElementById(id);if(row)row.remove();calcGrandTotal();}
...
function calcGrandTotal(){
    ...
    const taxRate=parseFloat(document.getElementById("invTax")?document.getElementById("invTax").value:0)||0;
...
async function saveInvoice(){
    ...
    // Get first item description as service name
    const firstDesc=document.querySelector(".inv-desc");
    const service=firstDesc?firstDesc.value.trim():"Service";
    ...
    try {
        ...
        if(typeof saveNotification==="function"){
            await saveNotification("invoice","New Invoice Created",
                (user?user.name:"Staff")+" created invoice "+number+" for "+clientName+" — ₦"+total.toLocaleString("en-NG"),
                "🧾",user?user.name:"Staff");
        }
        closeModal("invoiceCreateModal");
        resetInvoiceForm();
        renderInvoices();
        if(typeof renderInvoiceSection==="function") renderInvoiceSection();
        if(typeof showToast==="function") showToast("✅ Invoice "+number+" created!");
        else alert("✅ Invoice "+number+" created!");
    } catch(err){alert("Error creating invoice. Try again.");}
}

function resetInvoiceForm(){
    ["invClientName","invClientEmail","invClientPhone","invNotes"].forEach(id=>{const el=document.getElementById(id);if(el)el.value="";});
    const taxEl=document.getElementById("invTax");if(taxEl)taxEl.value="0";
    const today=new Date().toISOString().split("T")[0];
    const due=new Date(Date.now()+7*24*60*60*1000).toISOString().split("T")[0];
    const di=document.getElementById("invDate");const dd=document.getElementById("invDueDate");
    if(di)di.value=today;if(dd)dd.value=due;
    const container=document.getElementById("invoiceItems");
    if(container)container.innerHTML=`<div class="invoice-item-row" id="item-row-1"><div class="inv-item-fields">
...
async function renderInvoices(){
    ...
    try{
        ...
        if(invoices.length===0){tbody.innerHTML=`<tr><td colspan="8" class="empty-row">No invoices yet. Click "New Invoice" to create one.</td></tr>`;return;}
        tbody.innerHTML=invoices.map(inv=>`
            <tr>
                <td><strong>${inv.number}</strong></td>
                <td>${inv.client_name}</td>
                <td>${inv.service||"—"}</td>
                <td>${fmtInvDate(inv.date)}</td>
                <td><strong style="color:var(--green)">₦${Number(inv.total||0).toLocaleString("en-NG")}</strong></td>
                <td><span class="badge ${inv.status==="Paid"?"badge-active":"badge-pending"}">${inv.status}</span></td>
                <td><span class="added-by-tag ${inv.added_role==="admin"?"by-admin":"by-staff"}">${inv.added_by||"—"}</span></td>
                <td class="action-btns">
                    ...
                    <button class="erp-pw-btn"   onclick="toggleInvoiceStatus(${inv.id},'${inv.status}')">${inv.status==="Paid"?"Unpaid":"Mark Paid"}</button>
                    ${user&&user.role==="admin"?`<button class="erp-delete-btn" onclick="deleteInvoice(${inv.id})">Delete</button>`:""}
                </td>
            </tr>`).join("");
    }catch(err){tbody.innerHTML=`<tr><td colspan="8" class="empty-row">Error loading invoices.</td></tr>`;}
}
...
async function viewInvoice(id){
    try{
        ...
        if(preview)preview.innerHTML=buildInvoiceHTML(inv);
...
async function printInvoice(id){
    try{
        ...
        pw.document.write(`<!DOCTYPE html><html><head><title>Invoice ${inv.number} - YaseerTech</title>
        <style>
            *{margin:0;padding:0;box-sizing:border-box;}
            ...
            .inv-title p{font-size:14px;color:#666;margin-top:4px;}
...
function buildInvoiceHTML(inv,forPrint=false){
    ...
    return`
        ...
        <div class="inv-details">
            ...
            <div class="inv-meta">
                <p>Invoice Date: <strong>${fmtInvDate(inv.date)}</strong></p>
                ${inv.due_date?`<p>Due Date: <strong>${fmtInvDate(inv.due_date)}</strong></p>`:""}
            </div>
        </div>
        <table>
        ...
        <div class="totals-wrap">
            <table>
                <tr><td>Subtotal</td><td style="text-align:right">₦${Number(inv.subtotal||0).toLocaleString("en-NG",{minimumFractionDigits:2})}</td></tr>
                ${Number(inv.tax_rate||0)>0?`<tr><td>Tax (${inv.tax_rate}%)</td><td style="text-align:right">₦${Number(inv.tax||0).toLocaleString("en-NG",{minimumFractionDigits:2})}</td></tr>`:""}
                <tr class="grand-row"><td><strong>TOTAL</strong></td><td style="text-align:right"><strong>₦${Number(inv.total||0).toLocaleString("en-NG",{minimumFractionDigits:2})}</strong></td></tr>
            </table>
        </div>
...
async function toggleInvoiceStatus(id,currentStatus){
    ...
    try{
        ...
        renderInvoices();
        if(typeof renderInvoiceSection==="function") renderInvoiceSection();
        if(typeof showToast==="function") showToast("✅ Invoice marked as "+newStatus);
