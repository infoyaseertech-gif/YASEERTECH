// ============================================
//   YASEERTECH ERP - FINAL CLEAN VERSION
//   - Login + Forgot Password on login page
//   - Only Admin creates users inside dashboard
...
// ============================================================
// IMPORTANT: On first use, manually seed the admin account
// by opening browser console on login.html and running:
//   seedAdmin("Your Name","you@email.com","yourpassword")
// Then refresh and login normally. Do this ONCE only.
// ============================================================

// ========================
// SEED ADMIN (run once in console)
// ========================
function seedAdmin(name, email, password) {
    const users = JSON.parse(localStorage.getItem("yt_users") || "[]");
    if (users.length > 0) {
        console.warn("Admin already exists. Clear localStorage first if you want to reset.");
        return;
    }
    const admin = { id: Date.now(), name, email: email.toLowerCase().trim(), password, role: "admin", status: "active" };
    localStorage.setItem("yt_users", JSON.stringify([admin]));
    console.log("✅ Admin created! You can now login with:", email);
}
...
// DATA HELPERS
// ========================
function getUsers()      { return JSON.parse(localStorage.getItem("yt_users"))    || []; }
function saveUsers(u)    { localStorage.setItem("yt_users",    JSON.stringify(u)); }
...
// ========================
// RESET CODE STORE (in memory)
// ========================
const resetStore = {}; // { email: { code, expires } }

// ========================
// SHOW / HIDE VIEWS ON LOGIN PAGE
// ========================
...
function doLogin(e) {
    ...
    // Check admin exists — if no users at all, show guidance
    if (users.length === 0) {
        errBox.innerHTML = "⚠️ No accounts found. Open browser console (F12) and run:<br><code>seedAdmin('Your Name','email@gmail.com','password')</code>";
...
function sendResetCode() {
    ...
    if (!email) {
        showResetMsg(msgBox, "⚠️ Please enter your email address.", "error"); return;
    }
    ...
    if (!user) {
        showResetMsg(msgBox, "⚠️ No account found with that email address. Contact your administrator.", "error"); return;
    }
    ...
    // Show code on screen (works without email setup)
    showResetMsg(msgBox,
        `✅ Reset code generated.<br>
         ...
         <small style="color:#6B7280;">Write this down. It expires in 15 minutes.</small>`,
        "success"
    );
...
function confirmReset() {
    ...
    if (!stored) {
        showResetMsg(msgBox, "⚠️ Session expired. Please start again.", "error");
        return;
    }
    if (Date.now() > stored.expires) {
        showResetMsg(msgBox, "⚠️ Code has expired. Please request a new one.", "error");
        delete resetStore[email]; return;
    }
    if (entered !== stored.code) {
        showResetMsg(msgBox, "⚠️ Incorrect code. Please check and try again.", "error"); return;
    }
    if (!newPw || newPw.length < 6) {
        showResetMsg(msgBox, "⚠️ Password must be at least 6 characters.", "error"); return;
    }
    if (newPw !== confPw) {
        showResetMsg(msgBox, "⚠️ Passwords do not match.", "error"); return;
    }
...
function applyRoleUI() {
    ...
    // Hide admin-only elements from staff
    if (user.role !== "admin") {
...
document.addEventListener("click", e => {
    document.querySelectorAll(".erp-modal.open").forEach(m => {
...
function renderOverview() {
    ...
    if (rST) rST.innerHTML = rs.length === 0
        ? `<tr><td colspan="4" class="empty-row">No sales recorded yet</td></tr>`
    ...
    if (rET) rET.innerHTML = re.length === 0
        ? `<tr><td colspan="4" class="empty-row">No expenses recorded yet</td></tr>`
...
function addSale() {
    ...
    salesList.push({ id:Date.now(), customer, service, amount:Number(amount), date,
        addedBy: user ? user.name : "Unknown", addedByRole: user ? user.role : "unknown" });
...
function renderSales() {
    ...
    const now   = new Date();
    const month = salesList.filter(s => {
    ...
    if (salesList.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="empty-row">No sales yet. Click "Add Sale".</td></tr>`; return;
    }
    tbody.innerHTML = [...salesList].reverse().map((s,i) => `
        <tr>
            ...
            <td>${user&&user.role==="admin"
                ...
                : `<span style="color:var(--gray-text);font-size:12px">—</span>`}</td>
        </tr>`).join("");
}
...
function addExpense() {
    ...
    expensesList.push({ id:Date.now(), desc, category, amount:Number(amount), date,
        addedBy: user ? user.name : "Unknown", addedByRole: user ? user.role : "unknown" });
...
function renderExpenses() {
    ...
    const now   = new Date();
    const month = expensesList.filter(e => {
    ...
    if (expensesList.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="empty-row">No expenses yet. Click "Add Expense".</td></tr>`; return;
    }
    ...
    function buildReport(elId, list, key) {
        ...
        el.innerHTML = entries.length === 0
            ...
            : entries.sort((a,b) => b[1]-a[1])
                .map(([k,v]) => `<div class="report-item"><strong>${k}</strong><span>${fmtN(v)}</span></div>`)
                .join("");
    }
...
function saveUser() {
    ...
    if (id) {
        ...
        users.push({ id:Date.now(), name, email, password:pw, role, status });
    }
...
function clearUserForm() {
    ...
    ["editUserId","userName","userEmail","userPassword","userPasswordConfirm"]
...
function changeMyPassword() {
    ...
    if (!user || user.password !== oldPw) {
        showResetMsg(msgBox, "⚠️ Current password is incorrect.", "error"); return;
    }
    if (!newPw || newPw.length < 6) {
        showResetMsg(msgBox, "⚠️ New password must be at least 6 characters.", "error"); return;
    }
    if (newPw !== confPw) {
        showResetMsg(msgBox, "⚠️ New passwords do not match.", "error"); return;
    }
...
function saveService() {
    ...
    alert("✅ Service saved! It now appears on your website.");
}
...
function deleteService(id) {
    if (!confirm("Remove this service from your website?")) return;
...
function clearServiceForm() {
    ...
    ["editServiceId","serviceIcon","serviceTitle","serviceDesc","serviceBadge"]
        .forEach(id => { const el = document.getElementById(id); if (el) el.value = ""; });
}
